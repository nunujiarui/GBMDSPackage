...
initialFun.truncatedN <- function(model,cmds.result, dist.mat, metric){
  ...
  if (dist.metric == "cosine"){
    delta.mat <- 1-philentropy::distance(x.initial, method = dist.metric, mute.message = TRUE)
    colnames(delta.mat) =  rownames(delta.mat) <- rownames(d.mat)
  } else{
    delta.mat <- philentropy::distance(x.initial, method = dist.metric, mute.message = TRUE)
    colnames(delta.mat) =  rownames(delta.mat) <- rownames(d.mat)
  }
