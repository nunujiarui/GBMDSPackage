...
prior.truncatedN_incr <- function(model){
...
initialFun.truncatedN_incr <- function(model, prev.result, dist.mat, metric){
...
proposalFun.truncatedN_incr <- function(model, currentVal, n, dist.mat,
...
likelihoodFun.truncatedN_incr <- function(model, dist.mat, proposal.result, metric, n.incr){
