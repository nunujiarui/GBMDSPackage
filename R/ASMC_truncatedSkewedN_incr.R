...
truncatedSkewedN_incr <- function(hyperparList, p=2, reference.x.sd){
  bmdsmodel <- list()
  bmdsmodel$hyperparList <- hyperparList
  bmdsmodel$p <- p
  bmdsmodel$reference.x.sd <- reference.x.sd
  ## Set the name for the class
  class(bmdsmodel) <- append(class(bmdsmodel), c("truncatedSkewedN_incr", "BMDSModel"))
  return(bmdsmodel)
}
...
prior.truncatedSkewedN_incr <- function(model){
...
initialFun.truncatedSkewedN_incr <- function(model, prev.result, dist.mat, metric){
...
proposalFun.truncatedSkewedN_incr <- function(model, currentVal, n, dist.mat,
  ...
  if (sigma2.proposal <=  1e-10 | sigma2.proposal == Inf){
    ...
    proposal <- list(lambda = lambda.proposal, x = x.proposal,
                     sigma2 = sigma2.proposal, psi = psi.proposal)
...
dproposalFun.truncatedSkewedN_incr <- function(model, n, dist.mat,
...
likelihoodFun.truncatedSkewedN_incr <- function(model, dist.mat, proposal.result, metric, n.incr){
