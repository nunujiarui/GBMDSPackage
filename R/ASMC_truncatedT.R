...
truncatedT <- function(hyperparList, p=2, reference.x.sd){
  bmdsmodel <- list()
  bmdsmodel$hyperparList <- hyperparList
  bmdsmodel$p <- p
  bmdsmodel$reference.x.sd <- reference.x.sd
  ## Set the name for the class
  class(bmdsmodel) <- append(class(bmdsmodel), c("truncatedT", "BMDSModel"))
  return(bmdsmodel)
}

prior <- function(model) UseMethod("prior", model)

prior.truncatedT <- function(model){
...
initialFun.truncatedT <- function(model, cmds.result, dist.mat, metric){
...
proposalFun.truncatedT <- function(model, currentVal, n, dist.mat,
...
dproposalFun.truncatedT <- function(model, n, dist.mat,
...
likelihoodFun.truncatedT <- function(model, dist.mat, proposal.result, metric){
